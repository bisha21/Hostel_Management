...
const DiningScheduleView: React.FC = () => {
    ...
    return (
        <Card className="w-full">
            <CardHeader>
                <CardTitle className="text-2xl font-bold">Dining Schedule</CardTitle>
            </CardHeader>
            <CardContent>
                <Tabs defaultValue={activeDay} className="w-full">
                    ...
                    {days.map(day => (
                        <TabsContent key={day} value={day} className="mt-0">
                            <h3 className="text-xl font-semibold mb-4">{dayNames[day]}</h3>
                            ...
                            <div className="space-y-6">
                                {mealTypes.map(mealType => {
                                    ...
                                    return (
                                        <Card key={mealType} className="border-muted overflow-hidden">
                                            <div className="bg-muted p-4 flex items-center justify-between">
                                                <h4 className="text-lg font-medium">{mealNames[mealType]}</h4>
                                                {meal && (
